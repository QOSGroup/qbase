...
//币种名称统一为大写: https://github.com/QOSGroup/qbase/issues/140
...
func (coin Coin) SameDenomAs(other Coin) bool {
	return equalIgnoreCase(coin.Denom, other.Denom)
}
...
func (coin Coin) Plus(coinB Coin) Coin {
	...
	return Coin{strings.ToUpper(coin.Denom), coin.Amount.Add(coinB.Amount)}
}
...
func (coin Coin) Minus(coinB Coin) Coin {
	...
	res := Coin{strings.ToUpper(coin.Denom), coin.Amount.Sub(coinB.Amount)}
...
func (coins Coins) safePlus(coinsB Coins) Coins {
	...
	coins = mergeAndSortCoins(coins)
	coinsB = mergeAndSortCoins(coinsB)
	...
	for {
		...
		switch strings.Compare(strings.ToUpper(coinA.Denom), strings.ToUpper(coinB.Denom)) {
...
func (coins Coins) IsEqual(coinsB Coins) bool {
	...
	for i := 0; i < len(coins); i++ {
		if !equalIgnoreCase(coins[i].Denom, coinsB[i].Denom) || !coins[i].Amount.Equal(coinsB[i].Amount) {
			return false
		}
	}
...
func (coins Coins) AmountOf(denom string) BigInt {
	...
	case 1:
		coin := coins[0]
		if equalIgnoreCase(coin.Denom, denom) {
			return coin.Amount
		}
		return ZeroInt()
	...
	default:
		...
		if lessIgnoreCase(denom, coin.Denom) {
			return coins[:midIdx].AmountOf(denom)
		} else if equalIgnoreCase(denom, coin.Denom) {
			return coin.Amount
		} else {
...
func (coins Coins) Less(i, j int) bool {
	return lessIgnoreCase(coins[i].Denom, coins[j].Denom)
}
...
func lessIgnoreCase(s1, s2 string) bool {
	return strings.ToUpper(s1) < strings.ToUpper(s2)
}

func copyCoin(coin Coin) Coin {
	return Coin{
		Denom:  strings.ToUpper(coin.Denom),
		Amount: coin.Amount,
	}
}

func mergeAndSortCoins(coins Coins) Coins {
	...
	if len(coins) == 1 {
		return Coins{copyCoin(coins[0])}
	}

	coins = coins.Sort()
	unDupCoins := make([]Coin, 0, len(coins))
	...
	for {
		i++
		if i == len(coins) {
			unDupCoins = append(unDupCoins, copyCoin(coin))
			break
		}
		coini := coins[i]
		if !coin.SameDenomAs(coini) {
			unDupCoins = append(unDupCoins, copyCoin(coin))
			coin = coini
		} else {
			coin = coin.Plus(coini)
		}
	}
