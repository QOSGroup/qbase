...
func TestKeyManagement(t *testing.T) {
	...
	_, err = cstore.Get(n1)
	require.Error(t, err)
	i, _, err := cstore.CreateEnMnemonic(n1, p1)

	require.NoError(t, err)
	require.Equal(t, n1, i.GetName())
	_, _, err = cstore.CreateEnMnemonic(n2, p2)
	require.NoError(t, err)
...
func TestSignVerify(t *testing.T) {
	...
	i1, _, err := cstore.CreateEnMnemonic(n1, p1)
	require.Nil(t, err)

	i2, _, err := cstore.CreateEnMnemonic(n2, p2)
	require.Nil(t, err)
...
func TestExportImport(t *testing.T) {
	...
	info, _, err := cstore.CreateEnMnemonic("john", "secretcpw")
	require.NoError(t, err)
...
func TestExportImportPubKey(t *testing.T) {
	...
	// CreateEnMnemonic a private-public key pair and ensure consistency
	notPasswd := "n9y25ah7"
	info, _, err := cstore.CreateEnMnemonic("john", notPasswd)
	require.Nil(t, err)
...
func TestAdvancedKeyManagement(t *testing.T) {
	...
	_, _, err := cstore.CreateEnMnemonic(n1, p1)
	require.Nil(t, err, "%+v", err)
...
func TestSeedPhrase(t *testing.T) {
	...
	// make sure key works with initial password
	info, mnemonic, err := cstore.CreateEnMnemonic(n1, p1)
	require.Nil(t, err, "%+v", err)
	...
	newInfo, err := cstore.Derive(n2, mnemonic, p2, hd.FullFundraiserPath)
	require.NoError(t, err)
