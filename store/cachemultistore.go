...
func newCacheMultiStoreFromRMS(bms *baseMultiStore) cacheMultiStore {
	cms := cacheMultiStore{
		db:           NewCacheKVStore(dbStoreAdapter{bms.db}),
		stores:       make(map[StoreKey]CacheWrap, len(bms.stores)),
		keysByName:   bms.keysByName,
		traceWriter:  bms.traceWriter,
		traceContext: bms.traceContext,
	}

	for key, store := range bms.stores {
...
// Implements MultiStore.
//func (cms cacheMultiStore) GetKVStoreWithGas(meter types.GasMeter, config types.GasConfig, key StoreKey) KVStore {
